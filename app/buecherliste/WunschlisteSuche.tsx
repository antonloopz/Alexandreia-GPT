// app/buecherliste/WunschlisteSuche.tsx
//
// Suchfeld für die Wunschliste (09/2026, Pendenz "Doublettenerkennung +
// automatische Ergänzung von Buchdetails" — Suchfunktion ergänzend dazu,
// damit man ein Buch vor dem Hinzufügen erstmal selbst schnell in der
// eigenen Liste nachschlagen kann). Gleiches Muster wie
// app/notizen/NotizenSuche.tsx: die Seite selbst bleibt eine Server
// Component (liest ?suche= direkt aus searchParams, analog zum
// bestehenden ?kategorie=-Filter), Live-Tippen läuft hier debounced
// (300ms) über router.replace() in die URL. Ein vorhandener
// ?kategorie=-Filter bleibt beim Tippen erhalten.
...
export default function WunschlisteSuche({ initial }: { initial: string }) {
  ...
  // im Render statt eines Effekts (siehe react-hooks/set-state-in-effect,
  // gleiches Muster wie NotizenSuche.tsx).
  ...
  function aktualisieren(neuerWert: string) {
    ...
    timerRef.current = setTimeout(() => {
      ...
      router.replace(query ? `/buecherliste?${query}` : "/buecherliste");
    }, 300);
  }
